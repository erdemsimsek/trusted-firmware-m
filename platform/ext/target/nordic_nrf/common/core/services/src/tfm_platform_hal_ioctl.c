...
/* Cache of non-secure-requested System OFF retention, as a mask of 32 KiB RAM
 * sections (bit i => section i of MEMCONF.POWER[0]). The power-saving default is
 * "retain nothing"; the System OFF path drops all retention and re-enables only
 * this set. Rebuilt by the non-secure app each boot (System OFF is a cold start).
 */
static uint32_t s_ret_section_mask;

/* True only if [addr, addr+len) lies entirely within the non-secure RAM window.
 * addr is a RAM coordinate for nrfx; it is never dereferenced here.
 */
...
tfm_platform_hal_ram_ctrl_service(const psa_invec *in_vec, const psa_outvec *out_vec)
{
	 ...
	 * retention plan; no range to validate.
	 */
	...
	case TFM_RAM_CTRL_OP_POWER: {
		/* System ON power-down/up, applied immediately to MEMCONF.CONTROL.
		 * CONTROL powers RAM in whole 32 KiB sections, so restrict the action
		 * to sections lying entirely within the non-secure window. A byte
		 * range may legitimately end mid-section at NS_DATA_LIMIT (e.g. on
		 * nRF7120 the top section also holds reserved boot-ROM / CRACEN
		 * key-exchange RAM); rounding inward keeps that section powered and
		 * never powers down the partially-used section holding live data.
		 */
	...
	case TFM_RAM_CTRL_OP_RETAIN: {
		/* System OFF retention: record in the secure cache. The authoritative
		 * apply (drop all, then re-enable the cache) happens at System OFF.
		 */
